...
import com.example.app8.UITeacher.StudentListActivityTeacher;
...
public class TFLiteFaceRecognition implements FaceClassifier {
    ...
    public void register(String name, String dateOfBirth, Recognition rec, Bitmap face, String username) {
        if (connection != null) {
            String updateSQL = "UPDATE STUDENT_LIST SET name_student = ?, date_of_birth = ?, ImageData = ?, FaceVector = ?, Title = ?, Distance = ?, Id = ? WHERE username = ?";
            try (PreparedStatement preparedStatement = connection.prepareStatement(updateSQL)) {
                ...
                preparedStatement.setBytes(4, faceVectorBytes);
                preparedStatement.setString(5, rec.getTitle());
                preparedStatement.setFloat(6, rec.getDistance());
                preparedStatement.setString(7, rec.getId());
